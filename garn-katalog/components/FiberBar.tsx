...
export function FiberBar({ fibers }: { fibers: FiberComponent[] | null | undefined }) {
  ...
  return (
    <div>
      <div className="flex h-3 rounded-full overflow-hidden border border-stone">
        {fibers.map((f) => (
          <div
            key={f.fiber}
            style={{ width: `${f.percentage}%`, background: COLORS[f.fiber] ?? '#B8A888' }}
            title={`${fiberLabel(f.fiber)} ${f.percentage}%`}
          />
        ))}
      </div>
      <ul className="mt-2 text-xs text-bark flex flex-wrap gap-x-3 gap-y-1">
        {fibers.map((f) => (
          <li key={f.fiber}>
            <span
            ...
            />
            {fiberLabel(f.fiber)} {f.percentage}%
          </li>
        ))}
      </ul>
    </div>
