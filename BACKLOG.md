...
- **F5 (S)** "Brugt op"-subflow: bekræftelses-prompt ("antal nøgler nulstilles") + valgfri "hvad brugte du det til?"-note. Genbruger `BrugNoeglerModal`-mønster. **Detaljeret plan**: `~/.claude/plans/der-er-mange-ting-splendid-kernighan.md` (F5-sektion). **Beslutninger truffet (2026-04-27)**: kun Projekt er required i folde-ud (dato auto-default = i dag), greyscale-kort + "Brugt op"-badge, filter "På lager" skjuler Brugt op som standard, ny `<BrugtOpFoldeUd>`-komponent + `markYarnAsBrugtOp`-helper.
...
- **Kun hele nøgler i Garnlager — ingen kvarte/halve (2026-04-27, fra Hannah)** — antal-nøgler-feltet i Mit Garnlager skal kun acceptere **hele tal** (1, 2, 3 …). I dag kan brugeren vælge kvarte (¼) og halve (½) nøgler, men det skaber unødig kompleksitet og forvirring. Hvis brugeren har et delvist forbrugt nøgle, kan de skrive det i **noter-feltet** ("ca. ½ nøgle tilbage", "knap så meget på den ene"). Tjek `components/app/Garnlager.jsx` — input-typen og evt. dropdown/stepper-controllen skal ændres til integer-only. Bevar `min={0}` så ønskelistegarn (0 nøgler) fortsat virker.
