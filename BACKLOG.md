...
- ~~**Markér favorit i garn-katalog (2026-05-14, fra Hannah)**~~ — **shippet 2026-05-14**: hjerteknap på `/garn/[slug]`-detaljeside (kun for indloggede), favoritter samlet på `/mine-favoritter` med top-3 teaser + "Vis alle"-toggle alongside opskrift-favoritter. Migration `20260514000002_saved_yarns.sql` (PK user_id+yarn_id, CASCADE på begge FK'er, RLS + GRANT til authenticated). Bevidste forenklinger: ingen `lib/data/saved-yarns.ts`-helper (kun 1 brugssted), ingen HeartIcon-ekstraktion fra DropsKort (rule of 3), CSS-only "Vis alle"-toggle (ingen ekstra fetch).

- **Favorit-knap og -filter i Garnkatalog-overblikket (2026-05-14, fra Hannah)** — hjerteknap direkte på `YarnCard` i `/garn`-grid'et (i dag er knappen kun på detaljesiden `/garn/[slug]`). Dertil en "Kun favoritter"-chip i `YarnFilters`-filterbjælken, præcis som under Opskrifter. Infrastruktur eksisterer allerede (`saved_yarns`-tabel med RLS, `YarnFavoriteButton`-komponent, `/mine-favoritter`-side). Det der mangler: (1) hjerteknap på `YarnCard` — kræver refaktorering da `YarnCard` pt. er en ren `<Link>` og knap-i-link er ugyldig HTML; (2) "Kun favoritter"-toggle i `YarnFilters`; (3) server-side pre-fetch af favorit-IDs i `app/garn/page.tsx` (se mønster i `opskrifter/page.tsx`); (4) `lib/data/saved-yarns.ts` helper med `fetchSavedYarnIds`. Arkitekt-spørgsmål: omstrukturér `YarnCard` til `<article>` + separat `<Link>` + `<button>`, eller anden strategi? Kør med: `/ny-feature Favorit-knap og -filter i Garnkatalog-overblikket`

- **Markér yndlingsgarn i lageret** — simpel favoritmarkering for brugerens EGNE garn i `yarn_items` (separat fra katalog-favoritter ovenfor). Fra Excel: "Yndlingsgarner". Bruger ville fx kunne sortere garnlageret efter "favorit først".
