...
export default function IdeerClient({ userId, isAdmin }: Props) {
  // Brugere som ikke er logget ind: vis login-prompt.
  if (!userId) {
    return (
  ...
  return <IdeerBoard userId={userId} isAdmin={isAdmin} />
}

function IdeerBoard({ userId, isAdmin }: { userId: string; isAdmin: boolean }) {
    ...
    async function load() {
      ...
      if ((data?.length ?? 0) === 0 && isAdmin) {
        const { data: seeded } = await supabase.from('ideas')
          .insert(SEED_CARDS.map(c => ({ ...c, created_by: userId }))).select()
        organise(seeded ?? [])
      } else {
        organise(data ?? [])
      }
  ...
  async function addCard(colId: string) {
    ...
    const { data, error } = await supabase.from('ideas')
      .insert([{ column_id: colId, title: title.trim(), description: desc.trim(), position, created_by: userId }])
      .select().single()
